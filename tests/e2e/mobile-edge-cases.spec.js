...
test.describe('Mobile and UI Edge Cases', () => {
  ...
  test.describe('Gesture Conflicts', () => {
    test('touch tap should toggle task completion only once (no double-toggle)', async ({ page }) => {
      // Regression test for bug where touchend AND synthetic click both called handleTaskClick
      // causing the task to toggle twice (completed → uncompleted), appearing as if the tap didn't work
      await app.addTodayTask('Touch toggle test');

      // Verify task starts uncompleted
      const initiallyCompleted = await app.isTaskCompleted('Touch toggle test');
      expect(initiallyCompleted).toBe(false);

      // Simulate a TOUCH tap (touchstart + touchend + synthetic click)
      // This is different from mouse click - touch devices generate BOTH events
      const task = page.locator('.task-item:has-text("Touch toggle test")');
      ...
      await page.evaluate(({ x, y }) => {
        ...
        // Simulate touch sequence exactly as mobile browsers do it
        ...
        // Small delay (simulating finger lift)
        setTimeout(() => {
          ...
          // Synthetic click (generated by browser after touchend unless preventDefault is called)
          setTimeout(() => {
      ...
      // Task should be COMPLETED (single toggle), not back to uncompleted (double toggle)
      const isCompleted = await app.isTaskCompleted('Touch toggle test');
      expect(isCompleted).toBe(true);
    });

    test('consecutive touch taps should each toggle task state', async ({ page }) => {
      // Regression test: after first tap works, subsequent taps should also work
      await app.addTodayTask('Consecutive taps');

      const task = page.locator('.task-item:has-text("Consecutive taps")');
      const box = await task.boundingBox();

      // Helper to simulate touch tap
      const simulateTouchTap = async () => {
        await page.evaluate(({ x, y }) => {
          const element = document.elementFromPoint(x, y);
          const taskItem = element.closest('.task-item');
          if (!taskItem) return;
          ...
          const touchStart = new TouchEvent('touchstart', {
            ...
            touches: [new Touch({ identifier: Date.now(), target: taskItem, clientX: x, clientY: y })],
            targetTouches: [new Touch({ identifier: Date.now(), target: taskItem, clientX: x, clientY: y })]
          });
          taskItem.dispatchEvent(touchStart);

          setTimeout(() => {
            const touchEnd = new TouchEvent('touchend', {
              bubbles: true, cancelable: true,
              changedTouches: [new Touch({ identifier: Date.now(), target: taskItem, clientX: x, clientY: y })]
            });
            taskItem.dispatchEvent(touchEnd);

            setTimeout(() => {
              taskItem.dispatchEvent(new MouseEvent('click', {
      ...
      // First tap: should complete
      await simulateTouchTap();
      expect(await app.isTaskCompleted('Consecutive taps')).toBe(true);

      // Second tap: should uncomplete
      await simulateTouchTap();
      expect(await app.isTaskCompleted('Consecutive taps')).toBe(false);

      // Third tap: should complete again
      await simulateTouchTap();
      expect(await app.isTaskCompleted('Consecutive taps')).toBe(true);
    });
